namespace Mapbox.Unity.Map
{
	...
	public class CustomStyleBundle
	{
		...
		private void AssignAssets(StyleAssetPathBundle styleAssetPathBundle)
		{
			...
			for (int i = 0; i < materials.Length; i++)
			{
				...
				{
					GameObject.Destroy(tempMaterials[i]);
				}
			}
	...
	[Serializable]
	public class GeometryMaterialOptions : ModifierProperties, ISubLayerCustomStyleOptions, ISubLayerTexturing
	{
		...
		// Properties required by ISubLayerCustomStyleOptions
		public Material TopMaterial
		{
			get { return materials != null && materials.Length > 0 && materials[0].Materials.Length > 0 ? materials[0].Materials[0] : null; }
			set { if (materials != null && materials.Length > 0 && materials[0].Materials.Length > 0) materials[0].Materials[0] = value; }
		}

		public Material SideMaterial
		{
			get { return materials != null && materials.Length > 1 && materials[1].Materials.Length > 0 ? materials[1].Materials[0] : null; }
			set { if (materials != null && materials.Length > 1 && materials[1].Materials.Length > 0) materials[1].Materials[0] = value; }
		}
		...
		public void SetDefaultMaterialOptions()
		{
			...
			switch (style)
			{
				case StyleTypes.Light:
					SetMaterialColor(materials[0].Materials[0], lightStyleOpacity);
					SetMaterialColor(materials[1].Materials[0], lightStyleOpacity);
					break;
				case StyleTypes.Dark:
					SetMaterialColor(materials[0].Materials[0], darkStyleOpacity);
					SetMaterialColor(materials[1].Materials[0], darkStyleOpacity);
					break;
				case StyleTypes.Color:
					SetMaterialColorValue(materials[0].Materials[0], colorStyleColor);
					SetMaterialColorValue(materials[1].Materials[0], colorStyleColor);
					break;
				default:
					break;
			}
		...
		// Helper methods to support both Built-in RP and URP color properties
		private void SetMaterialColor(Material mat, float alpha)
		{
			if (mat == null) return;

			Color color;
			// Try URP property first (_BaseColor)
			...
			// Fallback to Built-in RP property (_Color)
		...
		private void SetMaterialColorValue(Material mat, Color colorValue)
		{
			if (mat == null) return;
